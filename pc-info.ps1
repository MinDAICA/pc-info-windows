...
Write-Host "`n💻 Tên máy: $($system.Name)" -ForegroundColor Yellow
Write-Host "📦 Model: $($system.Model)" -ForegroundColor Yellow

# CPU
$cpu = Get-CimInstance -ClassName Win32_Processor
Write-Host "`n🧠 CPU: $($cpu.Name)" -ForegroundColor Green

# RAM
$ramText = "🧮 RAM: {0:N2} GB" -f ($system.TotalPhysicalMemory / 1GB)
Write-Host "`n$ramText" -ForegroundColor Green

# Ổ cứng
$disks = Get-CimInstance -ClassName Win32_DiskDrive
foreach ($disk in $disks) {
    Write-Host "`n💾 Ổ đĩa: $($disk.Model)" -ForegroundColor Cyan
    Write-Host "   Loại: $($disk.MediaType)" -ForegroundColor Gray
    $diskSizeText = "   Dung lượng: {0:N2} GB" -f ($disk.Size / 1GB)
    Write-Host $diskSizeText -ForegroundColor Gray
}
...
foreach ($adapter in $netAdapters) {
    ...
    Write-Host "`n🌐 Card mạng: $($adapter.Name)" -ForegroundColor Cyan
    Write-Host "   MAC: $($adapter.MacAddress)" -ForegroundColor Gray
    Write-Host "   IP: $($ip.IPAddress)" -ForegroundColor Gray
}

# Domain
Write-Host "`n🏢 Domain: $($system.Domain)" -ForegroundColor Magenta

# Pin (nếu là laptop)
$battery = Get-CimInstance -ClassName Win32_Battery
if ($battery) {
    Write-Host "`n🔋 Pin:" -ForegroundColor Cyan
    ...
    Write-Host "`n🔋 Không tìm thấy thông tin pin (có thể là máy bàn)" -ForegroundColor DarkGray
}
...
foreach ($t in $thermal) {
    ...
    $tempText = "🌡️ Nhiệt độ CPU: {0:N1} °C" -f $tempC
    Write-Host "`n$tempText" -ForegroundColor Red
}

# Cổng kết nối (HDMI/VGA/Type-C) – không thể lấy trực tiếp bằng PowerShell
Write-Host "`n🔌 Cổng kết nối: Không thể xác định bằng PowerShell. Vui lòng kiểm tra thủ công hoặc dùng phần mềm như HWiNFO hoặc Speccy." -ForegroundColor DarkYellow
